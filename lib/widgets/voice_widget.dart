                    ...
                    ? Center(
                        child: Padding(
                          ...
                          child: AnimatedSwitcher(
                            duration: const Duration(milliseconds: 200), // 新child 显示时长
                            reverseDuration: const Duration(milliseconds: 100), // 旧child 显示时长
                            transitionBuilder: (Widget child, Animation<double> animation) {
                              return ScaleTransition(scale: animation, alignment: Alignment.center, child: child);
                            },
                            child: Text(
                              (_countTotal - _count).toString(),
                              key: ValueKey( (_countTotal - _count)),
                              style: TextStyle(
                                fontSize: 70.0,
                              ),
                            ),
    ...
    if (_timer!.isActive) {
      if (_count < 1) {
        CommonToast.showView(
            context: context,
            msg: '说话时间太短',
            icon: Icon(Icons.error,size: 80,color: Colors.deepOrange.withOpacity(0.5),));
        isUp = true;
      }
      _timer?.cancel();
      _count = 0;
